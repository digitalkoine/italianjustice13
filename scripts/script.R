...
html_legend <- 
"<i class='fa fa-user' aria-hidden='true'></i>Military<br/>
...
m <- leaflet() %>% 
  # Add basemap
  addTiles(urlTemplate = toned_basemap) %>% 
  ...
  # Add legend for region polygons
  addLegend("bottomleft", pal = pal, values = ~number_integer,
            title = "Giudicati",
            labFormat = labelFormat(suffix = " per region"),
            opacity = 1) %>%
